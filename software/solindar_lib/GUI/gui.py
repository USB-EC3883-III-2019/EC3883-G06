...
class SolindarGUI(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def logFun(self):
        for i in range(self.con.n):
            self.log.info(str(self.con.filter_on[self.con.currentPosition+i]) + ',' + \
            str((-1)*self.con.position_fifo[self.con.currentPosition+i] * self.pos_conv*(180/np.pi)) + ','  +\
            str(self.lidarproc[self.con.currentPosition+i]) + ',' + \
            str(self.sonarproc[self.con.currentPosition+i]) + ',')
